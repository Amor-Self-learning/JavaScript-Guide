...
Decorators are a way to modify or annotate classes and their members at design time. They provide a clean syntax for metaprogramming.

**Basic Syntax:**

```javascript
// Decorator is a function that receives metadata about the decorated element
function myDecorator(value, context) {
  console.log('Decorating:', context.name);
  // Return a replacement value or initialization function
  return value;
}
...
```javascript
// The context object contains metadata about what's being decorated
function logContext(value, context) {
  console.log('Context:', {
    kind: context.kind,        // 'class', 'method', 'getter', 'setter', 'field', 'accessor'
    name: context.name,        // Name of the element (string or symbol)
    static: context.static,    // true if static member
    private: context.private,  // true if private member
    access: context.access     // Object with get/set methods (when applicable)
  });
